...
class FixtureFormatter:
    ...
    def __fixture_info(self, fixture):
        ...
        if "league" in fixture:
            ...
            fixture_info['country_name'] = country_name
            flag_iso = {
                "England": "GB-ENG",
                "Scotland": "GB-SCT",
                "Wales": "GB-WLS",
            ...
            extra = country.get("extra")
            if country_name in flag_iso and extra:
                iso = flag_iso[country_name]
                extra["iso2"] = iso
            if extra:
                fixture_info['iso'] = extra["iso2"].lower()
            else:
                fixture_info['iso'] = None
    ...
    def __format_goals(fixture, important):
        ...
        return {
            ...
            "goals_after_75_no": not goals_after_75,
            'btts_no': btts_no,
    ...
    def __get_half_time_stats(self, stats_minute, item):
        try:
            home_item_trends = stats_minute["home"][item]
            away_item_trends = stats_minute["away"][item]
            home_item_ht = home_item_trends[45]
            away_item_ht = away_item_trends[45]
            return f"{home_item_ht}-{away_item_ht}"
    ...
    def format(self, fixture):
        ...
        keys = fixture.keys()
        if "goals" in keys and "corners" in keys and "cards" in keys and "stats" in keys:
            result = {
                **goals_data,
                **corners_data,
                **cards_data,
        ...
        if "trends" in fixture:
            ht_corner = self.__get_half_time_stats(stats_minute, "corners")
            if ht_corner:
                try:
                    data["result"]["ht_corner"] = ht_corner
                except:
                    data.update({"result.ht_corner": ht_corner})
            ht_score = self.__get_half_time_stats(stats_minute, "goals")
            if ht_score:
                try:
                    data["result"]["ht_score"] = ht_score
                except:
                    data.update({"result.ht_score": ht_score})
