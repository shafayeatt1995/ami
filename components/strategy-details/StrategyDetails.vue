<template>
  <StrategyDetailsLayout
    :page_title="filter ? filter.title : 'Loading...'"
      ...
      <div class="d-flex col-gap-12 wrap-on-mobile">
        ...
        >
          <div class="hit-rate" v-if="filter.type == 'pre-match'">
            ...
            <p :class="[$getColor(filter.hit_rate, 'text-')]">
              {{ filter.hit_rate === null ? 'NA' : filter.hit_rate.toFixed(2) }}
              %
            </p>
          </div>
          <div v-else>
        ...
        <div class="d-flex col-gap-12">
          <b-button
            class="footy-button"
            target="_blank"
            :href="`/api/user/strategies/picks/export?strategy_id=${$route.params.id}&type=${type}`"
          >
            <span>
...
import Track from '~/components/strategy-details/Track.vue';
  ...
  methods: {
    ...
    async getFilter(id) {
      try {
        ...
        if (filter.hit_rate === null) {
          this.timeout = setTimeout(() => {
            this.getFilter(id);
          }, WAIT_SECONDS * 1000);
          this.waited_seconds += WAIT_SECONDS;
        } else {
          this.waited_seconds = 0;
        }
      } catch (error) {
        console.log(error);
        this.$router.push('/');
