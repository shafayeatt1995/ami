<template>
  <b-overlay :show="loading" class="w-100">
    <div
      class="footy-page-container flex-col w-100"
      style="gap: 13px"
      v-if="$store.state.leagues"
    >
      <slot name="heading">
              ...
              >
                <div class="league-name-outcome">
                  ...
                  <div class="text-right">
                    ...
                    <h6
                      :class="
                        $getColor(getHitRate(league_id).percentage, 'text-')
                      "
                    >
                      {{ getHitRate(league_id).label }}
                      <!-- NA% (0/0) -->
                    </h6>
                  </div>
                </div>
...
export default {
  ...
  computed: {
    ...
    filteredCountries() {
      if (this.search_text.length === 0) return this.countries;
      const pattern = new RegExp(this.search_text, 'gi');
  ...
  methods: {
    ...
    getHitRate(value) {
      if (this.hits_lookup[value]) {
        const {hits, total} = this.hits_lookup[value];
        const percentage = ((hits / total) * 100).toFixed(2);
        return {
          hits,
          total,
          percentage,
          label: `${percentage}% (${hits}/${total})`,
        };
      } else {
        return {
          hits: 0,
    ...
    // filter(items, key) {
    //   if (this.search_text.length === 0) return items;
    //   const pattern = new RegExp(this.search_text, "gi");
