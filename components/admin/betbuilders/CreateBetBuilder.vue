...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch('fetchOutcomes', {is_bet_builder: true});
  },
  watch: {
  ...
  methods: {
    ...
    addRule() {
      ...
      if (this.setting.rule_id != null) {
        const {comparator, value, rule_id} = data;
  ...
  computed: {
    ...
    rules() {
      if (this.$store.getters.getPreMatchRuleById) {
        const rules = this.$store.getters.getPreMatchRuleById;
