<template>
  <div class="footy-vertical-checkbox">
    <div class="d-flex justify-content-between align-items-center py-2">
      <label for="" v-if="label" class="footy-label flex-grow-1">{{
        label
      }}</label>
      <div class="d-flex link-group">
        <label
          v-if="globalValue.length"
        ...
        <label
          v-if="outcomes"
          class="cursor-pointer text-primary footy-label ml-3"
          @click="checkAll"
        >
...
export default {
  props: {
    ...
    useGlobalLabel: {type: String, default: 'Use Global'},
    globalValue: {type: Array, default: () => []},
    label: String,
    filtered: Boolean,
    outcomes: Boolean,
  },
  ...
  computed: {
    ...
    values() {
      if (this.search_text.length === 0) return this.options;
      return this.options.filter((option) => {
        return (
          option.text.toUpperCase().indexOf(this.search_text.toUpperCase()) !=
          -1
        );
      });
    },
  ...
  methods: {
    ...
    useGlobal() {
      this.selected = this.globalValue;
    },
    checkAll() {
