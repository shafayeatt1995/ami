        ...
        <div class="mr-sticky box-inner">
          <div class="d-flex" style="align-items: center; column-gap: 10px">
            ...
            <h6
              class="text-primary"
              v-if="
                fixture.status in {LIVE: 1, HT: 1, PEN_LIVE: 1, BREAK: 1, ET: 1}
              "
            >
              {{ fixture.minute }}'
            </h6>
          ...
          <div class="fixture-timings mb-3">
            ...
            <span class="mr-2 text-grey" v-if="!is_live_mode">{{
              $moment
...
export default {
  ...
  data() {
    return {
      ...
      odd: null,
      fetch_fixture_interval: null,
    };
  },
  beforeMount() {
    if (!this.preloaded_fixture) {
      this.fetchFixture();
    } else {
      // console.log("FIXTURES SET FROM PROPS");
      this.$set(this, 'fixture', this.preloaded_fixture);
      this.fetchFixture();
      // this.getStats();
    }
  },
  beforeDestroy() {
    clearInterval(this.fetch_fixture_interval);
  },
  methods: {
    openh2h() {
  ...
  computed: {
    showLocationDropdown() {
      const stats_without_timings = ['General', 'Half', 'Cards'];
      const stats_with_timings = ['Goals', 'Corners'];
      return (
        this.scroller == 'Stats' &&
