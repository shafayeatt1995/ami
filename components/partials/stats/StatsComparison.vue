...
export default {
  ...
  methods: {
    extractData(key, location, team) {
      let stat_key;
      const options = pairs[key]['options'];
      if (options) {
        stat_key = options[location];
      } else {
        stat_key = key;
      }
      const data = this.fixture[team][stat_key];
      if (data != null) {
        return Number(data);
      } else {
        return 'N/A';
  ...
  computed: {
    fixtureStats() {
      const stats = statKeysGrouped[this.stat].map((item) => {
        if (typeof item === 'string') {
          ...
          const key = item.key;
          const home = this.extractData(key, this.location.home, 'home');
          const away = this.extractData(key, this.location.away, 'away');
          const overall = home + away;
          const label = pairs[key]['alias'] || pairs[key]['title'];
          const type = pairs[key]['type'];
          const active = pairs[key]['active'];
