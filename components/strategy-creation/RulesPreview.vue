<template>
  ...
  >
    ...
    <p
      v-if="strategy_type == 'pre-match-alerts' && rulesCount < 3"
      class="text-grey mb-3"
    >
      ⚠️ Atleast 3 rules are required
    </p>
    <p
      v-else-if="strategy_type == 'in-play-alerts' && strategyInplay.length < 1"
      class="text-grey mb-3"
    >
    ...
    >
      <div class="rule-item">
        ...
        <div class="rule-item-remover" v-if="!read_only">
          <b-button
            variant="white"
            size="sm"
...
export default {
  ...
  computed: {
    ...
    preMatchRules() {
      return this.$store.getters.getPreMatchRuleById;
    },
    inPlayRules() {
      return this.$store.getters.getInPlayRuleById;
    },
    strategyPrematch() {
