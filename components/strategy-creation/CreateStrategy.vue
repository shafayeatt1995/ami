<template>
  <CreateStrategyInner
    ...
    :preview="preview"
  >
    <template v-slot:categorySelection v-if="!is_inplay">
      <PreMatchBasic
        v-model="form"
        v-if="step == 0"
        @nextstep="nextStep"
    ...
    <template v-slot:ruleSelection>
      <div :class="step == 1 ? 'has-preview' : ''">
        ...
        <!-- <Timer v-model="form.timer" v-if="step == 1 && is_inplay"> </Timer> -->

        <component
          :is="is_inplay ? InPlayRules : PreMatchRules"
          v-if="step == 1"
          :category="category"
          v-model="form"
          @prevstep="prevStep"
        ...
        <div class="footy-page-container bottom-only" v-else>
          <SelectOutcomes
            ...
            :strategy_type="is_inplay ? 'in-play' : 'pre-match'"
            @prevstep="prevStep"
            @nextstep="nextStep"
          >
          </SelectOutcomes>
          <component
            :is="is_inplay ? InPlaySettings : PreMatchSettings"
            v-else
            v-model="form"
            @prevstep="prevStep"
            @nextstep="nextStep"
...
export default {
  ...
  methods: {
    ...
    async saveStrategy() {
      try {
        ...
        await this.$axios.saveStrategy(this.form, this.strategy_type);
        this.$bvToast.toast(
  ...
  computed: {
    is_inplay() {
      return this.strategy_type == 'in-play-alerts';
    },
    form: {
      get() {
        return this.value;
