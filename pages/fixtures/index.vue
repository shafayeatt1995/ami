              ...
              >
                ...
                <template v-if="search_results.length">
                  <b-list-group-item
                    ...
                    v-for="fixture in search_results"
        ...
        <b-row class="founds-container" no-gutters>
          <b-col md="6" sm="12" class="d-flex align-items-center">
            <h6 class="mb-3 m-md-0">
              {{ fixtures_count }} Match{{ fixtures_count > 1 ? "es" : "" }} Found
            </h6>
          </b-col>
          <b-col md="6" sm="12">
            <section class="">
              <div class="filter-items-wrapper scroll-on-mobile column-gap-10">
                ...
                <FixtureDatePicker v-model="selected_date" v-if="!is_live_mode" />
              </div>
            </section>
          </b-col>
          <b-col cols="12"> </b-col>
        ...
        <div class="fixture-list fixture-stats-section">
          ...
          <template v-else>
            <FixtureSorted
              ...
              @countChange="countChange"
              v-if="selected_settings.length > 0 || rule_filters.length"
...
import FixtureFavorite from "~/components/fixtures-page/FixtureFavorite.vue";
...
export default {
  ...
  data() {
    return {
      ...
      searchType: "upcoming",
      live_fixtures_interval: null,
      search_text: "",
      interval: null,
      rule_filters: [],
      is_search_loading: false,
      is_search_results_shown: false,
      filterer_id: null,
      search_types: [
  ...
  watch: {
    search_text(val) {
      ...
      if (val) {
        ...
        this.is_search_results_shown = false;
      }
    },
    searchType(val) {
      if (this.search_text) {
  ...
  mounted() {
    ...
    this.$store.dispatch("fetchFavoriteFixtures");
  },
  components: {
  ...
  methods: {
    ...
    countChange(value) {
      this.fixtures_count = value;
    },
    filterFixtures(rules) {
      this.rule_filters = rules;
      this.filterer_id = this.$uuid.v4();
    },
    closeStats() {
    ...
    async searchFixtures() {
      ...
      const search_results = await this.$axios.$get("/user/fixtures/search", {
        params: { search_text: this.search_text, type: this.searchType },
      });
      this.search_results = search_results;
      // this.$refs.searchDropdown.show(true);
      this.is_search_loading = false;
      this.is_search_results_shown = true;
    },
  },
};
</script>
